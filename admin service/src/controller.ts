...
export const deleteAlbum = trycatch(
    async (req: authenticatedRequest, res) => {
        ...
        const findAlbum = await sql`
            SELECT * FROM albums WHERE id = ${album_id}
        `

        if (!findAlbum || findAlbum.length == 0) {
            res.status(404).json({
                message: "No album with this album id"
            })
            return;
        }
...
export const deleteSong = trycatch(
    async (req: authenticatedRequest, res) => {
        ...
        const findSongId = await sql`SELECT * FROM songs WHERE id= ${song_id}`

        if (!findSongId || findSongId.length == 0) {
            res.status(404).json({
